...
printf("before locating all qualified elements\n");
        ...
        int comp_cnt = 0;
        int str_comp_cnt;
        int node_checked_cnt;
        locate_all_elements(lst, &key, &qualified_node_pointers, &qualified_node_pointer_count, &comp_cnt, &str_comp_cnt, &node_checked_cnt);
#ifdef debug_mode
...
#ifdef SEARCH_MODE
            printf("- comparisons: b%d n%d s%d\n", comp_cnt * BIT_PER_BYTE, node_checked_cnt, str_comp_cnt);
#endif
#ifdef REMOVE_MODE
            printf("deleted\n");
#endif
